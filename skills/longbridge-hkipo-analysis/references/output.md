...
| 招股书章节字段                     | Longbridge `ipo detail {SYMBOL}`（先 `longbridge ipo --help` 选子命令）+ WebSearch HKEXnews `{URL}` 补缺 | YYYY-MM-DD HH:MM | 招股书发布日         |
...
| 锁定期 / 解禁                | Longbridge `ipo detail {SYMBOL}`（先 `longbridge ipo --help`）+ WebSearch HKEXnews | YYYY-MM-DD HH:MM | 招股书披露     |
| {…按需追加…}                 | …                                                 | …                | …              |
