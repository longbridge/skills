...
When the `longbridge` CLI binary is missing, fall back to the Longbridge MCP server (`claude mcp add --transport http longbridge https://mcp.longbridge.com`). Discover available tools at runtime from the MCP tool list — do **not** hard-code tool names in this skill, they evolve with the server.
